...
def stars_from_survey(d,target_list=None):
    '''
    Read a schedule of stars to be surveyed from an external table

    Parameters
    ----------
    d : Table
        Dummy variable for table of planets
    target_list : Table, optional
        Table of stars to be surveyed. Generated by survey object

    Returns
    -------
    d : Table
        Table of stars for use in planet generation

    '''
    if target_list!=None and type(target_list==Table):
        d= target_list
        return d
    else:
        raise ValueError("Failed to read target list")
        return
